function y = ParabolicConvolve(x, y, l, r)
% ParabolicConvolve performs a Gacia-Vicente parabolic convolution on a dataset 
% using Fourier Transforms. The provided x/y data is first re-sampled to
% the resolution provided by r, then convolved against a positive parabolic
% distribution centered on x with radius l. The inputs l and r
...
% y = rand(1, 10);
%
% % Convolve using a Parabolic convolution with radius 2
% y2 = ParabolicConvolve(x, y, 2);
%
...
if nargin < 2
    if exist('Event', 'file') == 2
        Event('ParabolicConvolve requires at least two inputs, x and y', 'ERROR');
    else
        error('ParabolicConvolve requires at least two inputs, x and y');
    end
...
try
    ...
    % Calculate Parabolic distribution given radius l and centered on x
    g = max(real(sqrt(l^2 - (xi-mean(xi)).^2)/(pi*l^2)), 0); 
